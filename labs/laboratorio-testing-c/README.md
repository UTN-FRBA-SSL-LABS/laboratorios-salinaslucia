...
> R:inicializa las variables del carrito. es importante llamarla antes de usar el carrito para que no se lea basura.
...
> R:en la linea 24 y dice: total += c->items[i].precio;

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:debería multiplicar el precio por la cantidad
...
> R:falla, devuelve un 1 en vez de un 0
...
> R:si, las lineas 29 y 30 de la funcion carrito_descuento. no se ejecutarion porque la suite de tests unitarios no llama a la funcion directamente
...
> R:un test unitario aisla y prueba una funcion especifica para chequear que funcione bien de manera independiente, mientras que un test de integracion chequea como interactuan varios modulos juntos en un flujo real. el bug de carrito_total lo detectó primero el test unitario test_total_con_cantidad.
...
> R:c no hace verificacion de limites de arreglos en tiempo de ejecucion ni maneja memoria de manera administrada, por lo que escribir fuera de la memoria que reservas sobrescribe datos o causa un segmentation fault. en java o python esto tiraría un index error evitando que se arruine la memoria.
...
> R:automatizar las pruebas permite validar casos limite de manera repetible y rapida sin confiar en la persona que lee el codigo que puede saltarse errores facilemente. 
...
> R:porque cuando la cantidad es 1, multiplicar po 1 o no es lo mismo. el bug solo se aparece cuando la cantidad es mayor a 1.
